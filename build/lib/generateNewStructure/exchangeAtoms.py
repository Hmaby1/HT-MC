...
class ExchangeAtoms(object):
    ...
    def generate_new_structure(self, new_structure_index: int, elements_str_for_vaspkit:str,pick_first_specie:str=None,with_cutoff:bool=False):
        '''
        Description
        -----------
        ...
        if self.vac_dope: #空位文件写入
            new_structure = self._exchange(pick_first_specie,with_cutoff=with_cutoff)
    ...
    def pos_sort(self,structure,elements_str_for_vaspkit):
        ...
        sorted_indices.extend([i for i, specie in enumerate(species) if specie.symbol == self.vac_as])
