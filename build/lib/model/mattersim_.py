...
class mattersim_predict():
    ...
    def relax(self,
        ...
        if relax_model=='mattersim':
            ...
            while True:
                try:
                    ...
                    relax_result = relaxer.relax(contcar, steps=relax_step, fmax=fmax)  # 分子弛豫优化
        ...
        with open(self.output_path, "a+") as f: #保存优化过程
    ...
    def predict(self,
        ...
        #     potential = Potential.from_checkpoint(device=self.device)
        #     dataloader = build_dataloader(atom_lis, only_inference=True)
...
path = r"D:\Desk\work_file\RMC\SST1\0\POSCAR"
load_path = r'MatterSim-v1.0.0-5M.pth'
#a = mattersim_predict.load(poscar_path=path)#,load_path=load_path)
